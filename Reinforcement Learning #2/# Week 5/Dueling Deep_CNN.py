...
class DQN(nn.Module):
  def __init__(self, w, h, outputs):
    ...
    self.head  = nn.Linear(linear_input_size , 1)       # H(s)
    self.head2 = nn.Linear(linear_input_size , outputs) # S(s,a)
  ...
  def forward(self, x):
    ...
    H = self.head(x)          # data flatten 후 fc layer에 넣어주기
    S = self.head2(x)         # data flatten 후 fc layer에 넣어주기
...
def select_action(state):
    ...
    if sample > eps_threshold: # sample > 0.1
        with torch.no_grad(): # 그냥 뽑기만 할 거니까 학습 xxx
            ...
            return (Q(state)[0] + Q(state)[1] - Q(state)[1].mean()).max(1)[1].view(1, 1)
    else:
...
def Optimize():
    ...
    # 128개의 다음 State Sample에서 None이 아닌 것들 골라서 버리고 나머지 남기기 ---- 이것들만 모델에 넣어서 학습할 것 !
    ...
    Q_Values = (Q(batch_states)[0] + Q(batch_states)[1] - Q(batch_states)[1].mean()).gather(1, batch_actions)
    # Q_Values = Q(batch_states).gather(1, batch_actions)

    # None Mask에 의해서, 학습되지 않는 놈들은 0의 값으로 남아있을 것 ..
    next_state_values = torch.zeros(BATCH_SIZE , device = device)
    next_state_values[non_final_mask] = (Q_target(non_final_next_states)[0] + Q_target(non_final_next_states)[1] - Q_target(non_final_next_states)[1].mean()) .max(1)[0].detach()
