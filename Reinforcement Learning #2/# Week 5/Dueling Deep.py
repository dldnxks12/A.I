...
# 학습 OK 너무 느려 - batch size 변경
############################################################################
# Dueling Architecture이기 때문에 두 개의 model이 섞인 식이 존재
# 따라서 param = list(model1.parameters()) + list(model2.parameters())
# ---> torch.optim.Adam(param, lr = lr) 로 사용
