...
for episode in range(MAX_EPISODES):
    ...
    while not done: # Stacking Experiences
        ...
        # Voting
        actions = torch.stack([q_value_for_softmax4, q_value_for_softmax5, q_value_for_softmax6, q_value_for_softmax7, q_value_for_softmax8])
        action_softmax = torch.nn.functional.softmax(actions, dim=0).squeeze(1).squeeze(1)

        action_list = [action4, action5, action6, action7, action8]
...
np.save("./Rewards2", reward_history_20)
np.save("./Best Model Check2", avg_history)
