...
class QNet2(nn.Module):
    def __init__(self):
        ...
        self.fc_sA = nn.Linear(3, 32)  # State  24 개
        self.fc_aA = nn.Linear(1, 32)  # Action 4  개
        self.fc_qA = nn.Linear(64, 64)  # State , Action 이어붙이기
        self.fc_outA = nn.Linear(64, 1)  # Output : Q value

        self.fc_sB = nn.Linear(3, 32)  # State  24 개
        self.fc_aB = nn.Linear(1, 32)  # Action 4  개
...
q_optimizer2 = optim.Adam(q2.parameters(), lr=0.0003)
q_optimizer3 = optim.Adam(q3.parameters(), lr=0.003)
...
for episode in range(MAX_EPISODES):
    ...
    while not done:  # Stacking Experiences
        ...
        with torch.no_grad():
            if noise_index == 0:
                action1 = mu1(torch.from_numpy(state).float().to(device)) + noise
                action2 = mu2(torch.from_numpy(state).float().to(device))
                action3 = mu3(torch.from_numpy(state).float().to(device))
            elif noise_index == 1:
                action1 = mu1(torch.from_numpy(state).float().to(device))
                action2 = mu2(torch.from_numpy(state).float().to(device)) + noise
                action3 = mu3(torch.from_numpy(state).float().to(device))
            else:
                action1 = mu1(torch.from_numpy(state).float().to(device))
                action2 = mu2(torch.from_numpy(state).float().to(device))
                action3 = mu3(torch.from_numpy(state).float().to(device)) + noise

            q_value_for_softmax1 = q1(torch.from_numpy(state).float().unsqueeze(0).to(device), action1.unsqueeze(0))[0][0].unsqueeze(0)
            q_value_for_softmax2 = q2(torch.from_numpy(state).float().unsqueeze(0).to(device), action2.unsqueeze(0))[0][0].unsqueeze(0)
            q_value_for_softmax3 = q3(torch.from_numpy(state).float().unsqueeze(0).to(device), action3.unsqueeze(0))[0][0].unsqueeze(0)
...
plt.savefig('TD3 Pendulum5.png')

avg_history = np.array(avg_history)
np.save("./Pendulum", avg_history)
np.save("./Softmax_result", softmax_recores)