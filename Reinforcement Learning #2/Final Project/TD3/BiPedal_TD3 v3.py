...
class QNet1(nn.Module):
    ...
    def forward(self, x, a):
        h1A = F.relu(self.bn1(self.fc_sA(x)))
        h2A = F.relu(self.bn2(self.fc_aA(a)))
        catA = torch.cat([h1A, h2A], dim = 1)
        q1 = F.relu(self.bn3(self.fc_qA(catA)))
        q1 = self.fc_outA(q1)

        h1B = F.relu(self.bn1(self.fc_sB(x)))
        h2B = F.relu(self.bn2(self.fc_aB(a)))
        catB = torch.cat([h1B, h2B], dim = 1)
        q2 = F.relu(self.bn3(self.fc_qB(catB)))
        q2 = self.fc_outB(q2)
        return q1, q2
...
# Optimizer
mu_optimizer1 = optim.Adam(mu1.parameters(), lr=0.00003)
mu_optimizer2 = optim.Adam(mu2.parameters(), lr=0.0003)
mu_optimizer3 = optim.Adam(mu3.parameters(), lr=0.003)
q_optimizer1  = optim.Adam(q1.parameters(), lr=0.00003)
