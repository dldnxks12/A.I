...
class MuNet2(nn.Module):  # Output : Deterministic Action !
    def __init__(self):
        ...
        self.fc1 = nn.Linear(3, 32)  # Input  : 24 continuous states
        self.fc2 = nn.Linear(32, 32)
        self.fc_mu = nn.Linear(32, 1)  # Output : 4 continuous actions
...
class MuNet3(nn.Module):  # Output : Deterministic Action !
    def __init__(self):
        ...
        self.fc1 = nn.Linear(3, 128)  # Input  : 24 continuous states
        self.fc2 = nn.Linear(128, 128)
        self.fc_mu = nn.Linear(128, 1)  # Output : 4 continuous actions
...
class MuNet4(nn.Module):  # Output : Deterministic Action !
    def __init__(self):
        ...
        self.fc1 = nn.Linear(3, 96) # Input  : 24 continuous states
        self.fc2 = nn.Linear(96, 96)
        self.fc_mu = nn.Linear(96, 1) # Output : 4 continuous actions
...
class MuNet5(nn.Module):  # Output : Deterministic Action !
    def __init__(self):
        ...
        self.fc1 = nn.Linear(3, 192) # Input  : 24 continuous states
        self.fc2 = nn.Linear(192, 192)
        self.fc_mu = nn.Linear(192, 1) # Output : 4 continuous actions
...
class MuNet8(nn.Module):  # Output : Deterministic Action !
    def __init__(self):
        ...
        self.fc_mu = nn.Linear(256, 1) # Output : 4 continuous actions
...
mu_optimizer2 = optim.Adam(mu2.parameters(), lr=0.0003)
mu_optimizer3 = optim.Adam(mu3.parameters(), lr=0.003)
mu_optimizer4 = optim.Adam(mu4.parameters(), lr=0.0005)
mu_optimizer5 = optim.Adam(mu5.parameters(), lr=0.0003)
mu_optimizer6 = optim.Adam(mu6.parameters(), lr=0.003)
mu_optimizer7 = optim.Adam(mu7.parameters(), lr=0.0005)
mu_optimizer8 = optim.Adam(mu8.parameters(), lr=0.0003)
...
for episode in range(MAX_EPISODES):
    ...
    while not done:  # Stacking Experiences
        ...
        # Decaying Noise
        noise1 = ou_noise() * (DECAYING_RATE - episode * 0.05)
        noise2 = ou_noise() * (DECAYING_RATE - episode * 0.05)
        noise3 = ou_noise() * (DECAYING_RATE - episode * 0.05)
        noise4 = ou_noise() * (DECAYING_RATE - episode * 0.05)
        noise5 = ou_noise() * (DECAYING_RATE - episode * 0.05)
        noise6 = ou_noise() * (DECAYING_RATE - episode * 0.05)
        noise7 = ou_noise() * (DECAYING_RATE - episode * 0.05)
        noise8 = ou_noise() * (DECAYING_RATE - episode * 0.05)

        if (DECAYING_RATE - episode * 0.05) < 0:
            noise1, noise2, noise3, noise4, noise5, noise6, noise7, noise8 = 0, 0, 0, 0, 0, 0, 0, 0

        with torch.no_grad():
            action1 = mu1(torch.from_numpy(state).float().to(device)) + noise1
            action2 = mu2(torch.from_numpy(state).float().to(device)) + noise2
            action3 = mu3(torch.from_numpy(state).float().to(device)) + noise3
            action4 = mu4(torch.from_numpy(state).float().to(device)) + noise4
            action5 = mu5(torch.from_numpy(state).float().to(device)) + noise5
            action6 = mu6(torch.from_numpy(state).float().to(device)) + noise6
            action7 = mu7(torch.from_numpy(state).float().to(device)) + noise7
            action8 = mu8(torch.from_numpy(state).float().to(device)) + noise8


            q_value_for_softmax1 = q1(torch.from_numpy(state).float().unsqueeze(0).to(device), action1.unsqueeze(0))[0][0].unsqueeze(0)
            q_value_for_softmax2 = q1(torch.from_numpy(state).float().unsqueeze(0).to(device), action2.unsqueeze(0))[0][0].unsqueeze(0)
            q_value_for_softmax3 = q1(torch.from_numpy(state).float().unsqueeze(0).to(device), action3.unsqueeze(0))[0][0].unsqueeze(0)
            q_value_for_softmax4 = q1(torch.from_numpy(state).float().unsqueeze(0).to(device), action4.unsqueeze(0))[0][0].unsqueeze(0)
            q_value_for_softmax5 = q1(torch.from_numpy(state).float().unsqueeze(0).to(device), action5.unsqueeze(0))[0][0].unsqueeze(0)
            q_value_for_softmax6 = q1(torch.from_numpy(state).float().unsqueeze(0).to(device), action6.unsqueeze(0))[0][0].unsqueeze(0)
            q_value_for_softmax7 = q1(torch.from_numpy(state).float().unsqueeze(0).to(device), action7.unsqueeze(0))[0][0].unsqueeze(0)
            q_value_for_softmax8 = q1(torch.from_numpy(state).float().unsqueeze(0).to(device), action8.unsqueeze(0))[0][0].unsqueeze(0)
        ...
        action_list = [action1[0], action2[0], action3[0], action4[0], action5[0], action6[0], action7[0], action8[0]]
...
plt.savefig('TD3 Pendulum ensemble v2.png')

avg_history = np.array(avg_history)
np.save("./Pendulum2 ensemble v2", avg_history)
np.save("./ensemble Softmax_result v2", softmax_recores)