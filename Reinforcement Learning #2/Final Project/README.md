...
평균적으로 모델들이 150 Episode 동안 양의 Reward로 빠르게 올라가는 것을 알 수 있다.  
아래 그래프는 학습이 잘 된 모델을 포함한 그래프이다.

위 그래프에서 양의 Reward로 근접하는 것에 초점을 맞춘 이유는, 양의 평균 Reward가 나온 시점으로부터 학습이 굉장히 빨리 진행되었기 때문이다.
따라서 마지막 4개의 시행에서 모델들이 얼마나 빠르고 안정적으로 양의 평균 Reward로 접근하는지 확인해보았다. 
덧붙여 모델의 학습이 시간적으로 굉장히 부담이 컸기에, 최종 점수를 계산할 모델을 제외하고서는 모두 300 Episode 언저리에서 학습을 중단하였다. 
(학습이 충분히 마무리되었다고 판단)

최종적으로 학습한 모델을 이동 평균하며 그린 그래프는 다음과 같다.
