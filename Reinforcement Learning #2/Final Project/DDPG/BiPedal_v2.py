...
with open('DDPG_1.txt', 'w', encoding = 'UTF-8') as f:
    f.write("# ----------------------- # " + '\n')
...
plt.savefig('DDPG_1.png')# 코드 동작 OK
# 학습 X
###########################################################################
# To Avoid Library Collision
import os
...
while episode < MAX_EPISODES:
    ...
    while not done:
        ...
        ####################################################### 4개의 Action 조합에서 Softmax 분포로 뽑기 ...
        # Q Value를 기반으로 Softmax를 취해야 하므로 QNET은 1개 , MUNET은 4개 ...
        action = mu(torch.from_numpy(state).to(device))
        noise = torch.tensor(ou_noise(), device = device)
...
with open('DDPG_ensemble.txt', 'w', encoding = 'UTF-8') as f:
    f.write("# ----------------------- # " + '\n')
    f.write("DDPG_Parameter 2022-2-12" + '\n')
    f.write('\n')
    f.write('\n')
...
plt.xlabel("Episode")
plt.ylabel("Reward")
plt.title("DDPG_ensemble")
plt.plot(length, reward_history_20)
plt.savefig('DDPG_ensemble.png')