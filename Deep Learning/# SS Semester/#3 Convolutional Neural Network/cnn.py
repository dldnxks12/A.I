...
# Import Dataset
train_images = np.load("/content/drive/MyDrive/imageData_25.npy",      allow_pickle=True)
train_labels = np.load("/content/drive/MyDrive/imageDataLabel_25.npy", allow_pickle=True)
...
functions_list = [aug1, aug2, aug3, aug4, aug5, aug6, aug7, aug8, aug9, aug10, aug11, aug12, aug13, \
                  ...
                  aug26, aug27, aug28, aug29, aug30, aug31]
...
train_images, valid_images, train_labels, valid_labels = train_test_split( train_images, train_labels, test_size = 0.1, shuffle = True,  random_state = 100)
...
with tf.device('/gpu:0'):
  ...
  batch_size = 64
  history=model.fit(train_images, train_labels_onehot, batch_size=batch_size, epochs = 30, verbose=1)
...
#모델 저장
model.save("/content/drive/MyDrive/saved_model_vel")