...
try:
	while True:
		...
		#vertices = np.array([[(0,height),(width//2-150, height//2 + 30), (width//2+150, height//2 + 30), (width,height)]], dtype=np.int32)  
		...
		#lines     = cv2.HoughLinesP(canvas_Contour, 1, np.pi/180, 50, None, 30, 20)   # minGap = 30	       		
		lines     = cv2.HoughLinesP(canvas_Contour, 1, np.pi/180, 50, None, 1, 20)	# minGap = 10 
		#lines1     = cv2.HoughLinesP(canvas_Contour, 1, np.pi/180, 50, None, 30, 20)   # minGap = 30	       	

		# ------------------------- Initialize Angle ------------------------- #				
		if (lines is not None) and (Init_angle == True):
			print(" # --- Start Init Angle --- # ")
			...
			if len(L_lines) > 0:
				...
				cv2.line(canvas_Init, (x1, y1), (x2, y2), (255,255,255), 2)								
			...
			No_line_flag += 1
			if No_line_flag >= 3:
				Left_Wheel, Right_Wheel = 0, 0  # If there's no line for 3 times -> Stop  
		...
		if Init_angle == True:
			cv2.imshow("Init line", canvas_Init)					
		...
		# ------------------------ Initialize distance ---------------------- #	
			...
			if (len(Base_left_distance_temp) == 10) and (len(Base_right_distance_temp) == 10): 
		...
		if (lines is not None) and (Init_angle == False) and (Init_distance == False):
			...
			lines2 = lines2[np.abs(slope_degree) < 135]
			slope_degree = slope_degree[np.abs(slope_degree) < 135]			
