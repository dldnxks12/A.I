...
640 x 640 x 3 크기로 학습시켜줄 것이기 떄문에 모든 사진을 해당 크기로 바꿔주는 과정이 필요 
...
if ni.shape[0] >= ni.shape[1]:
    scale = ni.shape[0] / 640
    new_width = int(ni.shape[1] / scale)
    diff = (640-new_width) // 2
    ni = cv2.resize(ni, (new_width, 640))
    print(ni.shape)
    im[:, diff:diff+new_width,:] = ni

else:
    scale = ni.shape[1] / 640
    new_height = int(ni.shape[0] / scale)
    diff = (640 - new_height) // 2
    ni = cv2.resize(ni, (640, new_height))
